...
} from "../generated/BeaNFT-ABIs/basin";
import {
  ConsecutiveTransfer as ConsecutiveTransferEventBarnRaise,
  Transfer as TransferEventBarnRaise
} from "../generated/BeaNFT-ABIs/barnraise";
import { Transfer as TransferEventGenesis } from "../generated/BeaNFT-ABIs/genesis";
import { Transfer as TransferEventWinter } from "../generated/BeaNFT-ABIs/winter";
import { BeaNFTUser, CollectionData } from "../generated/schema";

const zeroAddress = "0x0000000000000000000000000000000000000000";

export function handleTransferGenesis(event: TransferEventGenesis): void {
  log.info("GENESIS TRANSFER! BEANFT: {}, RECEIVER: {}", [
...
export function handleTransferBarnRaise(event: TransferEventBarnRaise): void {
  log.info("BARN RAISE TRANSFER! BEANFT: {}, RECEIVER: {}", [
    event.params.tokenId.toI32().toString(),
    event.params.to.toHexString()
  ]);
  let from = event.params.from.toHexString();
...
export function handleConsecutiveTransferBarnRaise(event: ConsecutiveTransferEventBarnRaise): void {
  log.info("BARN RAISE CONSECUTIVE TRANSFER! FROM BEANFT {} TO {}, RECEIVER: {}", [
...
export function handleConsecutiveTransferBasin(event: ConsecutiveTransferEventBasin): void {
  log.info("BASIN CONSECUTIVE TRANSFER! FROM BEANFT {} TO {}, RECEIVER: {}", [
    event.params.fromTokenId.toString(),
    event.params.toTokenId.toString(),
    event.params.to.toHexString()
  ]);
...
function transferHandler(from: string, to: string, tokenId: i32, mode: string): void {
  ...
  if (source) {
    // If source is true this means it is a user wallet, as we make sure to not add the zero address as an user
    if (mode === "genesis") {
  ...
  if (destination) {
    // If true we have indexed the receiver as an user already, just update the arrays
    if (mode === "genesis") {
    ...
    // This is a new user, so initialize the arrays. This check also makes sure we don't index the zero address
