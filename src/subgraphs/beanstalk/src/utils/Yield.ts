...
import { BigDecimal_sum, f64_sum, f64_max } from "../../../../core/utils/ArrayMath";
...
import { getProtocolFertilizer, minEMASeason, stalkDecimals } from "../../../../core/constants/RuntimeConstants";
      ...
      if (gaugeSettings[i] !== null) {
        ...
        if (whitelistSettings[i].id == toAddress(beanstalk.token)) {
          ...
          germinatingNonGaugeBdv = [
            germinatingNonGaugeBdv[0].plus(germinating[0]),
            germinatingNonGaugeBdv[1].plus(germinating[1])
          ];
...
function updateFertAPY(protocol: Address, timestamp: BigInt, window: i32): void {
  ...
  if (t < 6534) {
    ...
    fertilizerYield.humidity = BigDecimal.fromString(
      currentFertHumidity.reverted ? "500" : currentFertHumidity.value.toString()
  ...
  fertilizerYield.deltaBpf = fertilizerYield.beansPerSeasonEMA.div(
    BigDecimal.fromString(fertilizerYield.outstandingFert.toString())
  );
    ...
    fertilizerYield.deltaBpf == ZERO_BD
      ...
      : fertilizerYield.humidity.div(
          BigDecimal.fromString("1")
            .plus(fertilizerYield.humidity)
            .div(fertilizerYield.deltaBpf)
            .div(BigDecimal.fromString("8760"))
        );
