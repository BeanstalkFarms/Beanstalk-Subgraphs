...
import { beans_BI as beans, podlineMil_BI } from "../../../core/tests/Values";
...
describe("Field: Plot Transfer", () => {
  ...
  describe("Full Plot", () => {
    ...
    test("F: Harvestable (Partial)", () => {
      ...
      assertFieldHas(
        ...
        initialPlots[0].pods.minus(harvestableAmount).plus(initialPlots[1].pods),
        harvestableAmount
      );
    });
  ...
  describe("Partial Plot: Middle", () => {
    ...
    test("M: Harvestable (Partial)", () => {
      ...
      assertFieldHas(
        BEANSTALK.toHexString(),
        initialPlots[0].pods.plus(initialPlots[1].pods).minus(harvestableAmount),
        harvestableAmount
      );
    });
