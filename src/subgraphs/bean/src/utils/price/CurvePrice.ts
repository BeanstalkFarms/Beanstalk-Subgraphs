...
import { BI_10, ONE_BI, toDecimal, ZERO_BD, ZERO_BI } from "../../../../../core/utils/Decimals";
import {
  BEAN_3CRV_V1,
  BEAN_LUSD_V1,
  CALCULATIONS_CURVE,
...
export function curvePriceAndLp(pool: Address): BigDecimal[] {
  ...
  if (pool == BEAN_3CRV_V1) {
    ...
    let lusd3crvPrice = toDecimal(
      lusd3PoolContract.get_dy(ZERO_BI, BigInt.fromI32(1), BigInt.fromString("1000000000000000000")),
      18
    ).div(amountAfterFee);
